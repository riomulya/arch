...
// Map untuk icon names ke components
const iconMap = {
  CheckCircleIcon,
  CurrencyDollarIcon,
  ShieldCheckIcon,
...
export default function AnimatedContent({ valuePropositions }) {
  // Map icon names ke components
  const mappedValuePropositions = valuePropositions.map((item) => ({
    ...item,
    icon: iconMap[item.iconName] || CheckCircleIcon,
  }));
  return (
    <>
      {/* Hero Section */}
      ...
      <section className='py-20 bg-white'>
        <div className='container mx-auto px-4 sm:px-6 lg:px-8'>
          ...
          <div className='grid md:grid-cols-2 lg:grid-cols-4 gap-8'>
            {mappedValuePropositions.map((item, index) => (
              <motion.div
                key={item.title}
